(() => {
  ...
  const STORAGE_KEY = 'turdgobbler-goblin-shop-v1';
  const ULTIMATE_STORAGE_KEY = 'turdgobbler-colourifier-ultimate-v2';
  ...
  const GLOBAL_BREW_KEYS = new Set([
  ...
  const els = {
    ...
    globalCauldron: $('globalCauldron'), cauldronContents: $('cauldronContents'),
  ...
  let pendingGlobalPurchase = false;
  ...
  function normaliseState() {
    ...
    if (state.selected?.kind === 'global' && !activeGlobals(state).some((entry) => entry.key === state.selected.key)) state.selected = null;
  }
  ...
  function restore() {
    ...
    try {
      ...
      if (!saved) saved = JSON.parse(localStorage.getItem(ULTIMATE_STORAGE_KEY) || 'null');
      if (!saved) {
  ...
  function emitShopReaction(type, detail = {}) {
    window.dispatchEvent(new CustomEvent('goblin-shop-reaction', {detail: {type, ...detail}}));
  }

  function rejectUnaffordable(before, candidate) {
    restoreSnapshot(before);
    persist();
    renderAll();
    const shortfall = Math.max(1, candidate.rawLength - candidate.limit);
    announce(`The goblin refuses the sale — ${shortfall} more character coin${shortfall === 1 ? '' : 's'} needed`, true);
    emitShopReaction('broke', {shortfall});
    return false;
  }
  ...
  function mutate(change, message = '', feedback = 'commit') {
    ...
    if (candidate.overLimit && candidate.rawLength > beforeBuild.rawLength) {
      history.pop();
      return rejectUnaffordable(before, candidate);
    }
    persist();
    renderAll();
    if (feedback) haptic(feedback);
    if (message) announce(message);
    emitShopReaction(feedback === 'delete' ? 'refund' : 'purchase', {message});
    return true;
  }
  ...
  function effectPayload(key) {
    ...
    if (key === 'br') return {kind: 'event', effectKey: key, event: {type: 'br'}};
    if (['italic', 'underline', 'strike', 'sup', 'sub'].includes(key)) {
      return {kind: 'event', effectKey: key, event: {type: 'tag', code: definition.code}};
    }
    if (['size', 'cspace', 'rotate', 'voffset', 'pos'].includes(key)) {
      const value = state.effects[key]?.value ?? definition.value;
      return {kind: 'event', effectKey: key, event: {type: 'tag', code: `<${key}=${Logic.shortestNumber(value, definition.value)}>`}};
    }
  ...
  function activeGlobals(source = state) {
    const entries = [];
    const formattingCodes = {italic: '<i>', underline: '<u>', strike: '<s>'};
    Object.entries(formattingCodes).forEach(([key, code]) => {
      if (source.formatting?.[key]) entries.push({key, label: EFFECT_BY_KEY[key].label, code});
    });
    ['sup', 'sub'].forEach((key) => {
      if (source.effects?.[key]) entries.push({key, label: EFFECT_BY_KEY[key].label, code: EFFECT_BY_KEY[key].code});
    });
    ['size', 'cspace', 'rotate', 'voffset', 'pos'].forEach((key) => {
      const setting = source.effects?.[key];
      if (!setting?.enabled) return;
      const definition = EFFECT_BY_KEY[key];
      entries.push({
        key,
        label: definition.label,
        code: `<${key}=${Logic.shortestNumber(setting.value, definition.value)}>`
      });
    });
  ...
  function renderOutput() {
    ...
    els.copyButton.disabled = !build.raw || over;
    ...
    els.outputStatus.textContent = over
      ? `${build.rawLength - build.limit} CHARACTER COINS SHORT`
      : `${build.limit - build.rawLength} COINS LEFT · ${build.colourStages}/${build.requestedColourStages} DYES BREWED`;
    els.copyLabel.textContent = over ? 'TOO EXPENSIVE' : '✓ TAKE IT';
  ...
  function draggableRange(label, value, minimum, maximum, step, apply, message, format = String) {
    ...
    range.addEventListener('change', () => {
      ...
      restoreSnapshot(before);
      const previousBuild = compile();
      restoreSnapshot(changed);
      if (candidate.overLimit && candidate.rawLength > previousBuild.rawLength) {
        const original = before;
        before = null;
        rejectUnaffordable(original, candidate);
        return;
      }
      history.push(before);
      if (history.length > MAX_HISTORY) history.shift();
      ...
      emitShopReaction('tune', {message: typeof message === 'function' ? message(next) : message});
    });
    setReadout();
  ...
  function applyGlobalBrew(payload, label) {
    const key = payload?.effectKey || eventKey(payload?.event || {});
    if (!GLOBAL_BREW_KEYS.has(key)) {
      announce('That charm needs an exact river position', true);
      emitShopReaction('nope', {message: `${label} cannot be brewed globally`});
      return false;
    }
    const purchased = mutate(() => {
      if (['italic', 'underline', 'strike'].includes(key)) {
        state.formatting[key] = true;
      } else if (['sup', 'sub'].includes(key)) {
        state.effects[key] = true;
        state.effects[key === 'sup' ? 'sub' : 'sup'] = false;
      } else {
        const definition = EFFECT_BY_KEY[key];
        const value = Number(eventValue(payload.event));
        state.effects[key] = {
          enabled: true,
          value: Number.isFinite(value) ? value : definition.value
        };
      }
      state.selected = {kind: 'global', key};
    }, `${label} brewed across the whole name`);
    if (!purchased) return false;
    pendingGlobalPurchase = false;
    els.globalCauldron.setAttribute('aria-expanded', 'false');
    document.body.classList.remove('cauldron-brewing');
    setActiveTab(null);
    requestAnimationFrame(() => document.querySelector(`[data-global-brew="${key}"]`)?.focus());
    return true;
  }

  function removeGlobalBrew(key) {
    mutate(() => {
      if (['italic', 'underline', 'strike'].includes(key)) state.formatting[key] = false;
      else if (['sup', 'sub'].includes(key)) state.effects[key] = false;
      else if (state.effects[key] && typeof state.effects[key] === 'object') state.effects[key].enabled = false;
      state.selected = null;
    }, `${EFFECT_BY_KEY[key]?.label || key} refunded to the character purse`, 'delete');
  }

  function renderCauldronContents() {
    els.cauldronContents.replaceChildren();
    const brews = activeGlobals().filter(({key}) => !['allCaps', 'smallCaps'].includes(key));
    if (!brews.length) {
      const empty = document.createElement('span');
      empty.className = 'cauldron-empty';
      empty.textContent = 'EMPTY POT';
      els.cauldronContents.appendChild(empty);
      return;
    }
    brews.forEach((brew) => {
      const button = document.createElement('button');
      button.type = 'button';
      button.dataset.globalBrew = brew.key;
      button.dataset.shopPrice = String(brew.code.startsWith('Aa') ? 0 : brew.code.length);
      button.innerHTML = `<b>${escapeHtml(brew.label)}</b><small>${escapeHtml(brew.code)}</small>`;
      button.setAttribute('aria-label', `${brew.label}, applied to the whole name. Press to tune or refund.`);
      button.addEventListener('click', () => {
        state.selected = {kind: 'global', key: brew.key};
        renderInspector();
        emitShopReaction('inspect', {message: `${brew.label} is bubbling nicely`});
      });
      els.cauldronContents.appendChild(button);
    });
  }

  function renderGlobalInspector(key) {
    const definition = EFFECT_BY_KEY[key];
    const brew = activeGlobals().find((entry) => entry.key === key);
    if (!definition || !brew) {
      state.selected = null;
      renderInspector();
      return;
    }
    els.layerInspector.replaceChildren(inspectorHeader('CAULDRON · WHOLE NAME', definition.label, brew.code));
    const body = document.createElement('div');
    body.className = 'inspector-controls cauldron-inspector';
    const fields = document.createElement('div');
    fields.className = 'inspector-fields';
    if (['size', 'cspace', 'rotate', 'voffset', 'pos'].includes(key)) {
      const setting = state.effects[key];
      fields.appendChild(draggableRange(
        'BREW STRENGTH',
        ...
        definition.step,
        (next) => {
          setting.value = Math.max(definition.min, Math.min(definition.max, next));
          setting.enabled = true;
          state.selected = {kind: 'global', key};
        },
        () => `${definition.label} brew strength updated`,
        (next) => String(Logic.shortestNumber(next, definition.value))
      ));
    } else {
      const note = document.createElement('p');
      note.className = 'inspector-note';
      note.textContent = 'THIS ENCHANTMENT AFFECTS THE ENTIRE NAME';
      fields.appendChild(note);
    }
    const actions = document.createElement('div');
    actions.className = 'inspector-actions';
    actions.appendChild(actionButton('REFUND BREW', 'delete-button', () => removeGlobalBrew(key)));
    body.append(fields, actions);
    els.layerInspector.appendChild(body);
  }
  ...
  function renderInspector() {
    ...
    if (selected.kind === 'global') return renderGlobalInspector(selected.key);
  ...
  function selectSourcePayload(payload, label) {
    if (pendingGlobalPurchase) {
      applyGlobalBrew(payload, label);
      return;
    }
  ...
  function appendFxOrbitButton({label, note = '', icon = '', className = '', action, effectKey = ''}) {
    ...
    if (effectKey) {
      button.dataset.effectKey = effectKey;
      const payload = effectPayload(effectKey);
      const code = payload?.event?.type === 'br' ? '<br>' : String(payload?.event?.code || '');
      if (code) button.dataset.shopPrice = String(code.length);
      renderFxExample(button, effectKey);
    }
    button.addEventListener('click', action);
  ...
  function appendFxGroup(keys) {
    keys.map((key) => EFFECT_BY_KEY[key]).filter(Boolean).forEach((effect) => {
      ...
      const button = appendFxOrbitButton({
        label: effect.label,
        note: effect.hint,
      ...
      const globallyAllowed = !pendingGlobalPurchase || GLOBAL_BREW_KEYS.has(effect.key);
      button.disabled = !payload || !globallyAllowed;
      if (!globallyAllowed) button.title = 'This charm needs an exact river position';
    });
  }
  ...
  function renderEffectSources() {
    ...
    if (fxMenuLevel === 'root') {
      els.fxOrbitStatus.textContent = pendingGlobalPurchase ? 'CAULDRON OPEN · CHOOSE A BREW FAMILY' : 'CHOOSE A CHARM FAMILY';
      ...
      els.fxOrbitStatus.textContent = `${group.label} ${pendingGlobalPurchase ? 'BREWS' : 'CHARMS'} · CHOOSE ONE`;
  ...
  function renderSpriteSources() {
    ...
    Array.from({length: 16}, (_, sprite) => sprite).forEach((sprite) => {
      ...
      button.dataset.shopPrice = String(`<sprite=${sprite}>`.length);
  ...
  function setActiveTab(name, focus = false) {
    ...
    const globalChoice = Boolean(
      pendingLayerOffset === null &&
      (['wubrg', 'presets'].includes(state.activeTab) || (pendingGlobalPurchase && state.activeTab === 'effects'))
    );
  ...
  function installEvents() {
    ...
    els.globalCauldron.addEventListener('click', () => {
      if (pendingGlobalPurchase) {
        cancelPendingLayer();
        setActiveTab(null);
        announce('Cauldron lid closed');
        return;
      }
      cancelPendingLayer();
      closeToolGuide();
      state.selected = null;
      pendingGlobalPurchase = true;
      els.globalCauldron.setAttribute('aria-expanded', 'true');
      document.body.classList.add('cauldron-brewing');
      fxMenuLevel = 'root';
      setActiveTab('effects');
      renderEffectSources();
      haptic('pluck');
      emitShopReaction('cauldron', {message: 'The whole-name cauldron is ready'});
      announce('Choose a charm to brew across the whole name');
    });
