...
export class BubbleWorld {
  ...
  addBubble(x, y, radius, newborn = false) {
    ...
    const b = {
      ...
      drag: null, cooldown: 0, wobble: 0, points: [],
    };
  ...
  spawn() {
    ...
    return this.addBubble(x, this.height + r * .35, r, true);
  }
  resize(width, height) {
  ...
  startDrag(id, x, y) {
    const b = this.get(id); if (!b || b.drag) return false;
  ...
  pop(id) {
    ...
    const event = { type: 'pop', x: b.x, y: b.y, radius: b.r, hue: b.hue, points: b.points.map(p => ({ x: b.x+p.x, y: b.y+p.y })) };
    ...
    for (const other of this.bubbles) {
      ...
      const kick = 95 * Math.exp(-d / (b.r*3));
      other.vx += dx/d*kick; other.vy += dy/d*kick; other.wobble = .9;
    }
    this.events.push(event); return event;
  }
  merge(a, b) {
    if (a.drag || b.drag || Math.hypot(a.r,b.r) > this.maxRadius) return null;
  ...
  tick(dt) {
    ...
    this.bubbles=this.bubbles.filter(b=>b.drag || b.y+b.r*2 > -40);
  ...
  deform(b,dt) {
    ...
    const motion=this.reducedMotion?.3:1, newborn=Math.max(0,1-b.age/2.8);
  ...
  collide(dt) {
    ...
    for(let i=0;i<this.bubbles.length;i++) for(let j=i+1;j<this.bubbles.length;j++) {
      ...
      if(a.drag||b.drag||a.held||b.held) {if(link)this.links.delete(key);continue;}
      if(d>sum*1.19) {if(link)this.links.delete(key);continue;}
      const canMerge=Math.hypot(a.r,b.r)<=this.maxRadius;
